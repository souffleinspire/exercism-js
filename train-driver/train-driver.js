...
export function fixListOfWagons(ids) {
  ...
  const newIds = [...everythingElse, a, b];
  return newIds;
}
...
export function correctListOfWagons(ids, missingWagons) {
  const [firstId, ...elseIds] = ids;
  const newIds = [firstId, ...missingWagons, ...elseIds];
  return newIds;
}
...
export function separateTimeOfArrival(information) {
  const {timeOfArrival, ...elseInformation} = information;
  return [timeOfArrival, elseInformation];
}
